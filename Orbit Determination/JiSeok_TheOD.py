...
def uncertainty(n, Combo): #calculate uncertainty by varying the RA and DEC of all three nights of observation by the uncertainties given by LSPR code
    ...
    for i in range((n+1)**6): #Make a list for each orbital element
...
a__ = 1.523652078812624
e__ = 9.352250586813468E-02
M__ = radians(2.729748585816485E+02)
Oprime__ = radians(4.951475194297512E+01)
iprime__ = radians(1.848417580912808)
wprime__ = radians(2.865823259837393E+02)

sqrtmu = 0.01720209895
mu = sqrtmu**2
time__ = 0
period__ = sqrt(4*pi**2*a__**3/mu) 
r1ecliptic__ = vector(0, 0, 0) 
Mtrue__ = 2*pi/period*(time__) + M__ 
Etrue__ = solvekep(Mtrue__)
position__ = np.array([a__*cos(Etrue__)-a__*e__, a__*sqrt(1-e__**2)*sin(Etrue__), 0])
r_w__ = np.array([[cos(wprime__), -sin(wprime__), 0],[sin(wprime__), cos(wprime__), 0],[0, 0, 1]])
r_i__ = np.array([[1, 0, 0],[0, cos(iprime__), -sin(iprime__)],[0, sin(iprime__), cos(iprime__)]])
r_O__ = np.array([[cos(Oprime__), -sin(Oprime__), 0],[sin(Oprime__), cos(Oprime__), 0],[0, 0, 1]])


rotation__ = np.dot(r_w__, r_i__, r_O__)
r1ecliptic__= np.dot(rotation__, position__)

asteroid__ = sphere(pos=r1ecliptic__*150, radius=(10), color=color.red) 
asteroid__.trail = curve(color=color.red) 

marslabel = label(pos=asteroid__.pos, text='Mars', xoffset=20, yoffset=12, space=asteroid_.radius, height=10, border=6, font='sans')
...
while (1==1): 
    ...
    Mtrue__ = 2*pi/period__*(time__) + M__
    Etrue__ = solvekep(Mtrue__)
    ...
    asteroid__.pos = r1ecliptic__*150 
    asteroid__.trail.append(pos=asteroid__.pos)
