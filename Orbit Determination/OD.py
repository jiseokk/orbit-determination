...
def uncertainty(n, Combo): #calculate uncertainty
    RA_error = 0.5214789432567722 / 3600 / 180 * pi #RA and DEC uncertainty from LSPR code
    DEC_error = 0.2774092111401255 / 3600 / 180 * pi
    
    RAoriginal = [4.21669293,  4.25379499,  4.31646395,  4.48544891,  4.52537259] 
    ...
    for i in range(n+1):
        for j in range(n+1):
            ...
            RA[Combo[1]] = (RAoriginal[Combo[1]] - RA_error) + (RA_error / n) * i * 2
            DEC[Combo[1]] = (DECoriginal[Combo[1]] - DEC_error) + (DEC_error / n) * j *2
    ...
    for i in range((n+1)**2): #Make a list for each orbital element
    ...
    print max(aSort), max(eSort), max(ISort), max(OmegaSort), max(wSort), max(MSort)
    elements = OD([0,2,3])
